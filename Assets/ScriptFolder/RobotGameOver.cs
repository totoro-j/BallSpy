...
public class RobotGameOver : MonoBehaviour {
	...
	void Update(){
		//如果遇到类型1警卫且当前机器人不在工作
		if(RobotGameOverTrigger.gameObject.GetComponent<RobotGameOverTrigger>().IsTriggered == true){
			if (RobotGameOverTrigger.gameObject.GetComponent<RobotGameOverTrigger>().IsGameOver.tag == "guard-1b" && isWork == false) {
				...
				if(JudgeOnce == true){
					...
					if (gameObject.transform.parent.gameObject == GameController.GetInstance().CurrentPlayer && Application.loadedLevelName != "DemoTech") {
						...
						if(gameObject.transform.parent.gameObject.name == "Robot_1-Body-3-SP"){
			...
			//如果遇到了警卫3并且还被控制状态
			if (RobotGameOverTrigger.gameObject.GetComponent<RobotGameOverTrigger>().IsGameOver.tag == "guard-3" && gameObject.transform.parent.gameObject == GameController.GetInstance().CurrentPlayer && Application.loadedLevelName != "DemoTech")
			{
				if (JudgeOnce == true)
				{
					foreach (Transform child in RobotGameOverTrigger.gameObject.GetComponent<RobotGameOverTrigger>().IsGameOver.transform.parent.transform.parent.transform)
