...
public class RobotGameOver : MonoBehaviour {
	...
	void OnTriggerStay(Collider IsRobotWork){
		//如果遇到类型1警卫且当前机器人不在工作
		if (IsRobotWork.tag == "guard-1b" && isWork == false) {
			IsRobotWorks = IsRobotWork.gameObject;
			if(JudgeOnce == true){
        ...
        //如果遇到了警卫3并且还被控制状态
        if (IsRobotWork.tag == "guard-3" && gameObject.transform.parent.gameObject == GameController.GetInstance().CurrentPlayer && Application.loadedLevelName != "DemoTech")
        {
            if (JudgeOnce == true)
            {
                foreach (Transform child in IsRobotWork.transform.parent.transform.parent.transform)
