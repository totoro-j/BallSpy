...
public class MineralAnimation : MonoBehaviour
{
	...
	public bool DestroyAlert = false;
    //图片ID
    private int id;
    ...
    private void FixedUpdate()
    {
        if (Active)
        {
            ...
            if (_timer.Update() == false)
            {
                ...
                //播放毁灭动画
                GameObject temp =
                    Instantiate(ParticleBoomPrefab, ParticleBoomTransform[3].position, Quaternion.identity) as
                        GameObject;
				DestroyAlert = true;
				StartCoroutine(Des(0.1f));
            }
            ...
            {
                GameObject temp =
                    Instantiate(ParticleBoomPrefab, ParticleBoomTransform[id].position, Quaternion.identity) as
                        GameObject;
                id++;
...
//                GetComponent<tk2dSprite>().
	...
	IEnumerator Des(float value) //等待的时间,单位秒  
	{   
		yield return new WaitForSeconds(value); 
		gameObject.transform.position = new Vector3(gameObject.transform.position[0], gameObject.transform.position[1], 50f);
	}
