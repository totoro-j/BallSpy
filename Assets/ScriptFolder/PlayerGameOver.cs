...
public class PlayerGameOver : MonoBehaviour {
	...
	void OnTriggerStay(Collider IsRobotWork){
		...
		if (IsRobotWork.tag == "guard-2b" && GameController.GetInstance().IsRobot == false) {
			if(JudgeOnce == true){
        ...
        //如果遇到了警卫3并且还被控制状态
        if (IsRobotWork.tag == "guard-3" && GameController.GetInstance().IsRobot == false)
        {
            if (JudgeOnce == true)
            {
                foreach (Transform child in IsRobotWork.transform.parent.transform.parent.transform)
        ...
        //如果遇到了二号电梯并且还被控制状态
        if (IsRobotWork.tag == "elevator2" && GameController.GetInstance().IsRobot == false)
        {
            if (JudgeOnce == true)
            {
                if (Application.loadedLevelName != "DemoTech")
