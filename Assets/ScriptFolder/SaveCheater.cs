...
public class SaveCheater : MonoBehaviour {
	...
	public void Cheater () {
		...
		if(GameController.GetInstance ().Levels.Count < 6){
			GameController.GetInstance ().Levels[2].isCurrent = false;
			GameController.GetInstance().Levels.Add(
				new Level{
				LevelNum = 6,
		...
		if(Global.GetInstance().SelectedSave == 1 && ES2.Exists ("player01.dat")){
			ES2.Save(6, "player01.dat?tag=CurrentLevelNum");
			ES2.Save(2, "player01.dat?tag=CurrentLevelScene");
			...
			ES2.Save(6, "player02.dat?tag=CurrentLevelNum");
			ES2.Save(2, "player02.dat?tag=CurrentLevelScene");
			...
			ES2.Save(6, "player03.dat?tag=CurrentLevelNum");
			ES2.Save(2, "player03.dat?tag=CurrentLevelScene");
