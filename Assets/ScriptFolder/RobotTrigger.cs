...
public class RobotTrigger : MonoBehaviour {
	...
	int start=1,length=8;//规定字符串截取的方法以获取机器人和工作台的前缀
			...
			if (gameObject.GetComponent<RobotGameOver> ().isAvail == true) {
				...
				if (GameController.GetInstance().IsRobot == true && gameObject == GameController.GetInstance().CurrentPlayerTrigger) {
					...
					if (IsElevatored == true) {
						...
						if(isGravity == false && ICanFly == false){
							isGravity = true;
					...
					if (GameController.GetInstance().intoWork == true && GetComponent<IsWorkSpace> ().isworkspace == true && gameObject == GameController.GetInstance().CurrentPlayerTrigger && GameController.GetInstance().CurrentPlayerTrigger.GetComponent<RobotGameOver> ().isWork == false) {
						...
						if (GameController.GetInstance().CurrentPlayer.name == "Robot_a2-Body-1") {
							//c型号机器人工作时有例外
							isNo2 = true;
					...
					if (GameController.GetInstance().moveUp[0] == 1 && GameController.GetInstance().IsRobot == true && ICanFly == true) {
						if (isMove == 1) {
							GameController.GetInstance().CurrentPlayer.transform.Translate (0f, 1.8f*Time.deltaTime, 0f);
						}
					}

					if (GameController.GetInstance().moveDown[0] == 1 && GameController.GetInstance().IsRobot == true && ICanFly == true) {
						if (isMove == 1) {
							GameController.GetInstance().CurrentPlayer.transform.Translate (0f, -1.8f*Time.deltaTime, 0f);
						}
					}
					...
					if (GetComponent<IsWorkSpace> ().isworkspace == true) {
                        ...
                        //2号机器人工作移动方式
                        if (RobotsName == "Robot_a2-1")
                        {
							Vector3 Robot2Position = transform.parent.transform.position;
                            if (_Turn)
                            {
								transform.parent.transform.position = new Vector3(Robot2Position.x + 0.03f, Robot2Position.y, Robot2Position.z);
                            }
                            else {
								transform.parent.transform.position = new Vector3(Robot2Position.x - 0.03f, Robot2Position.y, Robot2Position.z);
                            }
